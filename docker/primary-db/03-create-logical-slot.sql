DO $$
BEGIN
  IF (SELECT setting FROM pg_settings WHERE name = 'wal_level')::text = 'logical' THEN
    ...
    END IF;
  ELSE
    RAISE NOTICE 'wal_level is % - skipping logical slot creation', (SELECT setting FROM pg_settings WHERE name = 'wal_level');
  END IF;
